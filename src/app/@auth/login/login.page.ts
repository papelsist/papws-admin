  ...
  login() {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        this.service.claims$.pipe(take(1)).subscribe(async (claims) => {
          if (claims.xpapAdmin) {
            this.router.navigate(['/']);
          } else {
            console.log('Usuario no autorizado para este sistema...');
            this.service.singOut();
            await this.showMessage(
              'Acceso denegado',
              'Usted no tiene autorizado el acceso a esta a aplicación'
            );
          }
        });
      } catch (error) {
